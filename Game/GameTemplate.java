...
class GamePanel extends JPanel implements ActionListener, Runnable {
    ...
    public GamePanel(Dimension size1, MainPanel mp1) {
        ...
        bt1 = new JButton("ゲームクリア");
        bt1.setFont(f);
        bt1.addActionListener(this);
        int w1 = fm.stringWidth("ゲームクリア") + 40;
        int h1 = fm.getHeight() + 10;
        bt1.setSize(w1, h1);

        bt2 = new JButton("ゲームオーバー");
        bt2.setFont(f);
        bt2.addActionListener(this);
        int w2 = fm.stringWidth("ゲームオーバー") + 40;
        ...
        y = new int[3];
        for (int i = 0; i < 3; i++) {
            x[i] = rand.nextInt(size.width - 40);
            y[i] = rand.nextInt(size.height - 40);
        }
